// src/pages/Catalogos/Size/SizeManagement.tsx
...
export default function SizeManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        setSizes(
          sizes.map((size) =>
            size.id === editingId ? response.data : size
          )
        );
        setEditingId(null);
      } else {
  ...
  const handleEdit = (size: Size) => {
    setForm({ description: size.description });
    setEditingId(size.id);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const handleDelete = async (size: Size) => {
    if (!window.confirm(`¿Estás seguro de eliminar "${size.description}"?`)) {
      return;
    }
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              data-tour="size-search"
            />
            <div className="flex gap-2 sm:gap-3" data-tour="size-export">
              <Button
                onClick={exportExcel}
                theme="download"
                text="Excel"
