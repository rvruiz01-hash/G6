// src/pages/Catalogos/Color/ColorManagement.tsx
...
export default function ColorManagement() {
  ...
  const handleEdit = (color: Color) => {
    setForm({ description: color.description });
    setEditingId(color.id);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const handleDelete = async (color: Color) => {
    if (!window.confirm(`¿Estás seguro de eliminar "${color.description}"?`)) {
      return;
    }
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              data-tour="color-search"
            />
            <div className="flex gap-2 sm:gap-3" data-tour="color-export">
              <Button
                onClick={exportExcel}
                theme="download"
                text="Excel"
          ...
          <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
            Mostrando {filteredColors.length} de {colors.length}{" "}
            {colors.length === 1 ? "color" : "colores"}
          </div>
        </div>

        {/* TABLA */}
        <div data-tour="color-table">
          <Table
            data={filteredColors}
            columns={columns}
